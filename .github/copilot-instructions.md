...
```
src/data/mockData.ts  ──→  React pages  ──→  MapLibre GL + MapTiler (map rendering)
                                              ↕
...
- **Map config**: `src/lib/mapbox.ts` exports `MAPTILER_KEY`, `MAP_STYLE`, and `TERRAIN_SOURCE_URL` from `VITE_MAPTILER_KEY` env var. Used by `MapPage`, `PeakDetail`, `WaterfallDetail`, and `ContributionForm`
- **NEVER hardcode API keys** — all secrets go in `.env` (gitignored). Use `import.meta.env.VITE_*` to access them
- **Map container**: Must use explicit `width: 100%; height: 100%` — never `absolute inset-0` (MapLibre overrides `position` to `relative`)

### Build Optimization
`vite.config.ts` uses `manualChunks` to split heavy vendors:
- `vendor-core`: react, react-dom, react-router-dom
- `vendor-map`: maplibre-gl (~1MB, lazy-loaded only on Map page)
...
- **Frontend build vars**: `VITE_API_URL=https://api.bdpeaks.info`, `VITE_MAPTILER_KEY=<from MapTiler dashboard>`
